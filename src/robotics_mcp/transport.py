"""
FastMCP 3.4.4+ Dual Transport Configuration
...
Environment Variables:
    ...
    MCP_PORT: Port for HTTP/SSE. Default: 10706 (fleet 10700+; set MCP_PORT to override)
...
ENV_PORT = "MCP_PORT"  # default: 10706
...
def get_transport_config() -> dict:
    ...
    return {
        ...
        "port": int(os.getenv(ENV_PORT, "10706")),
...
Examples:
  ...
  # HTTP mode (web apps)
  python -m {server_name.replace("-", "_")} --http --port 10706

  # Via environment
  MCP_TRANSPORT=http MCP_PORT=10706 python -m {server_name.replace("-", "_")}
""",
    )

    transport_group = parser.add_mutually_exclusive_group()
    transport_group.add_argument("--stdio", action="store_true", help="Run in STDIO (JSON-RPC) mode (default)")
    transport_group.add_argument("--http", action="store_true", help="Run in HTTP Streamable mode (FastMCP 3.4.4+)")
    ...
    parser.add_argument("--port", type=int, default=None, help=f"Port to listen on (default: ${ENV_PORT} or 10706)")
...
def run_server(
    ...
    *,
    ...
    Args:
        ...
        show_banner: If False, suppresses FastMCP startup banner (required for stdio/IDE).

    Raises:
        Exception: If server fails to start.
    """
    asyncio.run(run_server_async(mcp_app, args, server_name, show_banner=show_banner))
    ...
    *,
    ...
    try:
        if transport == "stdio":
            ...
            await mcp_app.run_stdio_async(show_banner=show_banner)
