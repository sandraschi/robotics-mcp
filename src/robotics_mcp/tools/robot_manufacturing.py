...
class RobotManufacturingTool:
    ...
    async def _handle_3d_printer_control(self, device_id: str, action: str, **kwargs) -> Dict[str, Any]:
        ...
        if action == "start_print":
            if not file_path:
                return format_error_response("file_path required for start_print", error_type="validation_error")
            ...
            return await self._octoprint_home_axes(device_id, position.get("axes", ["x", "y", "z"]))
            ...
            if not gcode:
                return format_error_response("gcode required for send_gcode", error_type="validation_error")
            ...
            return format_error_response(f"Unknown 3D printer control action: {action}",
                                       error_type="validation_error")

    async def _handle_3d_printer_monitor(self, device_id: str, action: str, **kwargs) -> Dict[str, Any]:
        ...
        if action == "get_status":
            ...
            return format_error_response(f"Unknown 3D printer monitor action: {action}",
                                       error_type="validation_error")

    async def _handle_3d_printer_maintenance(self, device_id: str, action: str, **kwargs) -> Dict[str, Any]:
        """Handle 3D printer maintenance operations."""
        maintenance_action = kwargs.get("maintenance_action")
        ...
        if action == "level_bed":
            ...
            return format_error_response(f"Unknown 3D printer maintenance action: {action}",
                                       error_type="validation_error")
    ...
    async def _octoprint_get_temperatures(self, device_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return handle_tool_error("_octoprint_get_temperatures", e, device_id=device_id)
    ...
    async def _octoprint_set_temperature(self, device_id: str, temperature: Dict[str, float]) -> Dict[str, Any]:
        return format_success_response(f"Temperature set: {temperature}", data={"device_id": device_id})

    async def _octoprint_home_axes(self, device_id: str, axes: List[str]) -> Dict[str, Any]:
        return format_success_response(f"Axes homed: {axes}", data={"device_id": device_id})

    async def _octoprint_move_head(self, device_id: str, position: Dict[str, float]) -> Dict[str, Any]:
        return format_success_response(f"Head moved: {position}", data={"device_id": device_id})

    async def _octoprint_send_gcode(self, device_id: str, gcode: str) -> Dict[str, Any]:
        return format_success_response(f"G-code sent: {gcode[:50]}...", data={"device_id": device_id})
    ...
    async def _octoprint_unload_filament(self, device_id: str) -> Dict[str, Any]:
        return format_success_response("Filament unloaded", data={"device_id": device_id})

    async def _octoprint_get_files(self, device_id: str) -> Dict[str, Any]:
        return format_success_response("Files retrieved", data={"device_id": device_id, "files": []})

    async def _octoprint_level_bed(self, device_id: str) -> Dict[str, Any]:
        return format_success_response("Bed leveling started", data={"device_id": device_id})

    async def _octoprint_calibrate_pid(self, device_id: str) -> Dict[str, Any]:
        return format_success_response("PID calibration started", data={"device_id": device_id})

    async def _octoprint_clean_nozzle(self, device_id: str) -> Dict[str, Any]:
        return format_success_response("Nozzle cleaning started", data={"device_id": device_id})
