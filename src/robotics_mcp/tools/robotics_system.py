...
class RoboticsSystemTool:
            ...
            return format_success_response(
                ...
                data={
                    ...
                    "description": (
                        ...
                        "Primary: Dreame D20 Pro vacuum. Also Moorebot Scout, Yahboom ROSMASTER, Unitree (when available), "
                        "and virtual robots in Unity/VRChat. Integrates with osc-mcp, unity3d-mcp, vrchat-mcp, avatar-mcp."
                    ),
                    "features": [
        ...
        try:
            ...
            for server_name, server_instance in self.mounted_servers.items():
                try:
                    ...
                    logger.warning(
                        ":ERROR: Mounted server connectivity test failed",
                        server=server_name,
                        error=str(e),
                    )
                    mounted_servers_status[server_name] = {
            ...
            if self.config.enable_http:
                try:
                    ...
                    logger.warning(":ERROR: HTTP server status check failed", error=str(e))
                    http_status = {
                        "enabled": True,
                        "host": self.config.http_host,
                        "port": self.config.http_port,
            ...
            if robots_count > 0:
                ...
                recommendations.append(
                    "No robots currently registered - spawn virtual robots or connect physical robots"
                )
    ...
    async def _handle_list_robots(self, robot_type: str | None, is_virtual: bool | None) -> dict[str, Any]:
        ...
        try:
            ...
            logger.info(":SCAN: Robots listed", count=robot_count, filters=filter_desc)
