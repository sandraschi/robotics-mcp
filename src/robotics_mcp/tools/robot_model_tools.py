...
class RobotModelTools:
        ...
        async def robot_model(
            ...
            try:
                if operation == "create":
                    ...
                    if not script:
                        return format_error_response(
                            "script is required for 'execute_script' operation",
                            error_type="validation_error",
                        )
                    return await self._handle_execute_script(script, output_path, format)
                elif operation == "list_scripts":
                    return await self._handle_list_scripts()
                elif operation == "execute_repository_script":
                    if not category or not script_name:
                        return format_error_response(
                            "category and script_name are required for 'execute_repository_script' operation",
                            error_type="validation_error",
                        )
                    return await self._handle_execute_repository_script(category, script_name, output_path, format)
                else:
    ...
    async def _handle_create(
        ...
        try:
            supported_types = {"scout", "scout_e", "go2", "g1", "robbie", "custom"}
            if robot_type not in supported_types:
                return format_error_response(
                    f"Unsupported robot_type: {robot_type}. Supported: {', '.join(sorted(supported_types))}",
                    error_type="validation_error",
                )
        ...
        try:
            if "unity" in self.mounted_servers:
                ...
                return format_unavailable_error(
                    "Unity MCP",
                    "import model",
                    details={"robot_type": robot_type, "model_path": model_path, "format": format},
                )

        except Exception as e:
            logger.error("Unity import failed", robot_type=robot_type, error=str(e))
            return format_error_response(f"Unity import failed: {e!s}", error_type="import_error")
    ...
    async def _export_from_unity(
        ...
        @self.mcp.tool()
        async def create_moorebot_scout(
            output_path: str,
            format: Literal["fbx", "glb", "obj", "blend"] = "fbx",
            dimensions: dict[str, float] | None = None,
        ) -> dict[str, Any]:
            """Create Moorebot Scout robot model using Gemini-generated Blender script.
            ...
            Based on Gemini AI-generated Blender Python script that creates
            accurate mecanum wheel geometry with 45-degree roller angles.

            Args:
                output_path: Path where to save the created model.
                format: Output format (fbx, glb, obj, or blend).
                dimensions: Optional custom dimensions (length, width, height in meters).

            Returns:
                Success confirmation with model details and export information.

            Example:
                # Create default Scout model
                await create_moorebot_scout("D:/models/scout.fbx")

                # Create with custom dimensions
                await create_moorebot_scout(
                    "D:/models/scout_large.fbx",
                    dimensions={"length": 0.15, "width": 0.12, "height": 0.10}
                )
            """
            try:
                # Use default Scout dimensions if not provided
                if dimensions is None:
                    dimensions = self._get_default_dimensions("scout")

                # Gemini-generated Moorebot Scout Blender script
                scout_script = f"""import bpy
import math
...
# Create Chassis ({dimensions["length"] * 1000:.0f}x{dimensions["width"] * 1000:.0f}x{dimensions["height"] * 1000:.0f}mm)
bpy.ops.mesh.primitive_cube_add(size=1, location=(0, 0, {dimensions["height"] / 2 * 100}))
scout = bpy.context.active_object
scout.dimensions = ({dimensions["length"] * 100}, {dimensions["width"] * 100}, {dimensions["height"] * 100})
...
# Add the 4 Mecanum Wheels
# Offset: X +/- {dimensions["length"] / 2 * 100:.0f}, Y +/- {dimensions["width"] / 2 * 100:.0f}
wheel_y_offset = {dimensions["width"] / 2 * 100 - 15:.1f}
wheel_z = 25

create_mecanum_wheel("FL", ({dimensions["length"] / 2 * 100 - 15:.1f}, wheel_y_offset, wheel_z), True)   # Front Left
create_mecanum_wheel("FR", (-{dimensions["length"] / 2 * 100 - 15:.1f}, wheel_y_offset, wheel_z), False) # Front Right
create_mecanum_wheel("BL", ({dimensions["length"] / 2 * 100 - 15:.1f}, -wheel_y_offset, wheel_z), False) # Back Left
create_mecanum_wheel("BR", (-{dimensions["length"] / 2 * 100 - 15:.1f}, -wheel_y_offset, wheel_z), True)  # Back Right

# Add the Camera "Eye"
eye_z = {dimensions["height"] * 100 - 15:.1f}
bpy.ops.mesh.primitive_uv_sphere_add(radius=12, location=(0, {dimensions["width"] / 2 * 100 + 15:.1f}, eye_z))
camera = bpy.context.active_object
camera.name = "Scout_Eye"
"""

                # Execute the script using our new execute_script functionality
                result = await self._handle_execute_script(scout_script, output_path, format)
                    ...
                    result["data"].update(
                        {
                            "robot_type": "scout",
                            "model_features": [
                                "Mecanum wheels with 45° roller orientation",
                                "Rounded chassis with bevel modifier",
                                "Camera eye sensor mount",
                                "Proper wheel positioning and parenting",
                            ],
                            "dimensions": dimensions,
                            "wheel_type": "mecanum",
                            "wheel_count": 4,
                            "sensor_count": 1,
                            "generated_by": "Gemini AI + Moorebot Scout specs",
                        }
                    )
                ...
                return handle_tool_error("create_moorebot_scout", e, output_path=output_path, format=format)
...
import os
blend_path = r"{Path(output_path).with_suffix(".blend")!s}"
...
"""
            ...
            try:
                ...
                if blend_path.exists():
                    ...
                    if not blend_path_before or blend_mtime_after > blend_mtime_before:
                        ...
                        logger.warning("TBBmalloc warning detected, but continuing if file exists")
                    else:
                        raise
                elif "TBBmalloc" in error_str:
            ...
            return format_error_response(f"Model creation failed: {e!s}", error_type="creation_error")

    async def _create_textures_via_gimp(self, robot_type: str, texture_style: str) -> list[str]:
        """Create placeholder textures via gimp-mcp HTTP sim-art pipeline."""
        ...
        try:
            ...
            from ..utils.fleet_http_client import DEFAULT_GIMP_URL, call_gimp_tool, check_fleet_health

            if not await check_fleet_health(DEFAULT_GIMP_URL):
                logger.warning("gimp-mcp HTTP not reachable; skipping texture creation")
                return texture_paths

            texture_dir = Path("D:/Textures") / robot_type
            texture_dir.mkdir(parents=True, exist_ok=True)
            source_dir = Path(tempfile.mkdtemp(prefix=f"{robot_type}_tex_"))
            base = source_dir / f"{robot_type}_base.png"

            color_map = {
                "realistic": (90, 90, 95, 255),
                "stylized": (120, 80, 200, 255),
                "simple": (180, 180, 180, 255),
            }
            rgba = color_map.get(texture_style, color_map["realistic"])
            Image.new("RGBA", (1024, 1024), rgba).save(base)

            batch = await call_gimp_tool(
                "gimp_sim_art_tool",
                {
                    "operation": "gazebo_model_icons",
                    "input_dir": str(source_dir),
                    "output_dir": str(texture_dir),
                    "template_id": "gazebo_decal_512",
                    "validate": False,
                },
            )
            if not batch.get("success"):
                logger.warning("GIMP HTTP texture batch failed", detail=batch)
                return texture_paths

            files = batch.get("files") or batch.get("data", {}).get("files") or []
            texture_paths.extend(str(p) for p in files)
            logger.info(
                "Created textures via gimp-mcp HTTP",
                robot_type=robot_type,
    ...
    async def _convert_via_blender(
        ...
        try:
            ...
            return format_error_response(f"Blender conversion failed: {e!s}", error_type="conversion_error")
    ...
    async def _handle_execute_script(self, script: str, output_path: str | None, format: str) -> dict[str, Any]:
        """Execute custom Blender Python script (like Gemini-generated scripts)."""
        try:
            # Access blender-mcp directly from mounted servers
            blender_server = self.mounted_servers.get("blender")
            if not blender_server:
                return format_error_response(
                    "blender-mcp server not available for script execution",
                    error_type="not_available",
                    details={"mounted_servers": list(self.mounted_servers.keys())},
                )

            # Use blender-mcp's execute_script tool
            from blender_mcp.utils.blender_executor import get_blender_executor
            ...
            if not result.get("success", False):
                return format_error_response(
                    f"Blender script execution failed: {result.get('error', 'Unknown error')}",
                    error_type="script_execution_error",
                    details=result,
                )

            # If output_path is provided, save the scene
            if output_path:
                save_result = await executor.save_scene_async(output_path, format)
                if not save_result.get("success", False):
                    logger.warning(f"Failed to save scene to {output_path}: {save_result.get('error')}")
            ...
            return format_success_response(
                ...
                data={
                    ...
                    "scene_saved": bool(output_path),
                    "output_path": output_path,
                    "format": format,
                    "note": "Script execution completed. Check Blender for results.",
                },
            )

        except Exception as e:
            logger.error(f"Failed to execute Blender script: {e}")
            return format_error_response(f"Script execution failed: {e}", error_type="execution_error")

    async def _handle_list_scripts(self) -> dict[str, Any]:
        """List all available scripts in the repository."""
        try:
            import json
            from pathlib import Path
            ...
            if not index_file.exists():
                return format_error_response(
                    f"Script index not found at {index_file}",
            ...
            # Extract script listings by category
            categories = {}
            for category_name, category_data in index_data.get("categories", {}).items():
                scripts = {}
                for script_name, script_data in category_data.get("scripts", {}).items():
                    scripts[script_name] = {
            ...
            return format_success_response(
                "Script repository catalog retrieved",
                data={
                    "categories": categories,
                    "metadata": index_data.get("metadata", {}),
                    "repository_path": str(scripts_dir),
                },
            )

        except Exception as e:
            logger.error(f"Failed to list scripts: {e}")
            return format_error_response(f"Failed to list scripts: {e}", error_type="repository_error")

    async def _handle_execute_repository_script(
        self, category: str, script_name: str, output_path: str | None, format: str
    ) -> dict[str, Any]:
        """Execute a script from the repository."""
        try:
            import json
            from pathlib import Path
            ...
            if category not in categories:
                return format_error_response(
                    f"Category '{category}' not found. Available: {list(categories.keys())}",
                    error_type="invalid_category",
                )

            scripts = categories[category].get("scripts", {})
            if script_name not in scripts:
                return format_error_response(
                    f"Script '{script_name}' not found in category '{category}'. Available: {list(scripts.keys())}",
            ...
            # Execute the script
            result = await self._handle_execute_script(script_content, output_path, format)
            ...
            logger.error(f"Failed to execute repository script {category}/{script_name}: {e}")
            return format_error_response(f"Repository script execution failed: {e}", error_type="repository_error")
