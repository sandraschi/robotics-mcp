...
class RobotBehaviorTool:
    ...
    def register(self):
        ...
        async def robot_behavior(
            ...
            end_effector_pose: dict[str, Any] | None = None,
            gripper_position: float | None = None,
            arm_id: str | None = None,
            force_limit: float | None = None,
            manipulation_speed: float | None = None,
        ) -> dict[str, Any]:
            ...
            try:
                ...
                logger.error(
                    f"Error in robot behavior {category}.{action}",
                    robot_id=robot_id,
                    error=str(e),
                    exc_info=True,
                )
                ...
                if category == "navigation" and ("ros" in error_str or "master" in error_str):
                    ...
                    recovery_options = [
                        ...
                        "Try restarting the Robotics MCP server",
    ...
    async def _handle_virtual_navigation(
        ...
        try:
            if robot.platform == "unity" and "unity" in self.mounted_servers:
                ...
                return format_unavailable_error(
                    "Unity MCP",
                    f"run navigation action '{action}'",
                    robot_id=robot.robot_id,
                    action=action,
                    platform=robot.platform,
                )
    ...
    async def _handle_virtual_manipulation(
        ...
        try:
            if robot.platform == "unity" and "unity" in self.mounted_servers:
                ...
                return format_unavailable_error(
                    "Unity MCP",
                    f"run manipulation action '{action}'",
                    robot_id=robot.robot_id,
                    action=action,
                    platform=robot.platform,
                )
