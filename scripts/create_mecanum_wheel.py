...
def create_scout_with_mecanum_wheels():
    ...
    bpy.ops.mesh.primitive_cube_add(
        size=1, location=(0, 0, scout_height / 2), scale=(scout_length, scout_width, scout_height)
    )
    ...
    # Create wheels
    wheels = []
    for _i, (pos, name) in enumerate(zip(wheel_positions, wheel_names, strict=False)):
    ...
    with contextlib.suppress(Exception):
        bpy.ops.view3d.view_all()  # Skip if in background mode
