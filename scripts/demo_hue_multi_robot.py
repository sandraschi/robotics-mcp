...
async def demo_hue_safety_integration():
    ...
    try:
        ...
        if hue_status.get("success"):
            ...
            for _sensor_id, sensor_data in sensors.items():
                print(
                    f"   • {sensor_data['name']} ({sensor_data['location']}) - {'Enabled' if sensor_data['enabled'] else 'Disabled'}"
                )
        else:
        ...
        print(
            f"   🚨 MOVEMENT DETECTED: {simulated_movement['location']} (confidence: {simulated_movement['confidence']})"
        )
