...
async def test_robot_animation_robot_not_found(robot_animation_tool):
    ...
    tool_func = robot_animation_tool.mcp.tool.registered_func
...
async def test_robot_animation_get_state(robot_animation_tool, registered_robot):
    ...
    with patch("robotics_mcp.tools.robot_animation.call_mounted_server_tool", new_callable=AsyncMock) as mock_call:
...
@pytest.mark.asyncio
async def test_robot_animation_unavailable_without_unity(robot_animation_tool, state_manager):
    """Virtual animation fails clearly when Unity MCP is not mounted."""
    state_manager.register_robot("virtual_robot", "scout", platform="unity")
    robot_animation_tool.mounted_servers = {}

    tool_func = robot_animation_tool.mcp.tool.registered_func
    result = await tool_func(robot_id="virtual_robot", action="get_animation_state")

    assert result["status"] == "error"
    assert result["error_type"] == "not_available"
    assert "unity" in result["message"].lower()
