...
from robotics_mcp.utils.state_manager import RobotState, RobotStateManager
...
def test_robot_state_physical():
    ...
    robot = RobotState(robot_id="scout_01", robot_type="scout", connected=False)
...
def test_robot_state_virtual():
    ...
    robot = RobotState(robot_id="vbot_01", robot_type="scout", platform="unity")
...
def test_state_manager_register_with_metadata():
    ...
    metadata = {"position": {"x": 1.0, "y": 2.0, "z": 0.0}, "scale": 1.0, "spawned": True}

    robot = manager.register_robot("vbot_01", "scout", platform="unity", metadata=metadata)
