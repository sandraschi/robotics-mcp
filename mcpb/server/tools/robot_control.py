...
from ..utils.error_handler import format_error_response, handle_tool_error
...
class RobotControlTool:
    ...
    async def _handle_physical_robot(
        ...
        """
        try:
            logger.info("Physical robot command (unimplemented)", robot_id=robot.robot_id, action=action)
            return format_error_response(
                "Physical robot control requires ROS bridge integration (not implemented)",
                error_type="not_implemented",
                robot_id=robot.robot_id,
                action=action,
                details={
                    "required": "ROS bridge or robot-specific client (Yahboom, Elegoo, Dreame)",
                    "config": "Add robotics.<robot_type> config in config.yaml with connection details",
                },
            )
        except Exception as e:
